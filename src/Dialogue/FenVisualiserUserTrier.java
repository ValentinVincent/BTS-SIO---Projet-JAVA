 ...
 * @author DELL
 */
public class FenVisualiserUserTrier extends javax.swing.JFrame {
    // On fait appel au controler de ModeleFicheFrais qui prend en paramètre le "IdEtat" qu'on rentre en dur
   private ModeleFicheFrais leModeleFicheFrais = new ModeleFicheFrais("","","");
    /**
     * Creates new form FenMenuPrincipal
     */
    // On change le format du contenu des cellules du tableau
    public FenVisualiserUserTrier() {
        initComponents();
        complementGUI();
        Visiteur lesVisiteurs = new Visiteur();
        Etat lesEtat = new Etat();
        jComboBox1.addItem(" ");
        jComboBox2.addItem(" ");
        jComboBox3.addItem(" ");
        Mois leMois = new Mois();
        ArrayList<String> lesMois = leMois.liste();
        for(int i =0; i < lesVisiteurs.getLesEnrg().size();i++) {
            jComboBox1.addItem(lesVisiteurs.getLesEnrg().get(i).getIdVisiteur()+" "+lesVisiteurs.getLesEnrg().get(i).getPrenom()+" "+lesVisiteurs.getLesEnrg().get(i).getNom());
    }
        for (int i=0; i < lesEtat.getLesEnrg().size(); i++) {
            jComboBox2.addItem(lesEtat.getLesEnrg().get(i).getIdEtat()+" "+lesEtat.getLesEnrg().get(i).getLibelle());
        }
        
        for (int i=0; i < lesMois.size(); i++) {
            jComboBox3.addItem(lesMois.get(i).toString());
        }
        /*
    ...
    private void maj(){
        String leVisiteur = jComboBox1.getSelectedItem().toString();
        String leMois = jComboBox3.getSelectedItem().toString();
        String leEtat = jComboBox2.getSelectedItem().toString();
        String idVisiteur = "";
        String mois = "";
        String etat = "";
        if (!leVisiteur.equals(" ")){
            int blanc = leVisiteur.indexOf(" ");
            idVisiteur = leVisiteur.substring(0, blanc);
        }
        
        if (!leEtat.equals(" ")) {
            int blanc2 = leEtat.indexOf(" ");
            etat = leEtat.substring(0, blanc2);
        }
        
        if (!leMois.equals(" ")) {
            Mois lm = new Mois();
            mois = lm.Nummois(leMois);
        }
        ModeleFicheFrais leModeleFicheFrais = new ModeleFicheFrais(etat,idVisiteur,mois);
    ...
    private void ouvrir(){
     int numLigne = jTable1.getSelectedRow();
        ...
        FenVisualiserFiche laFenetre1 = new FenVisualiserFiche("", visiteur, lemois);
    ...
    private void initComponents() {
        ...
        getContentPane().add(jComboBox1, new org.netbeans.lib.awtextra.AbsoluteConstraints(1010, 120, 240, -1));
        ...
        getContentPane().add(jComboBox2, new org.netbeans.lib.awtextra.AbsoluteConstraints(1010, 210, 240, -1));
        ...
        getContentPane().add(jComboBox3, new org.netbeans.lib.awtextra.AbsoluteConstraints(1010, 300, 240, -1));
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(380, 0, 620, 660));
        ...
        jLab_Fond2.setPreferredSize(new java.awt.Dimension(1178, 786));
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                new FenVisualiserUserTrier().setVisible(true);
            }
        });
    }
